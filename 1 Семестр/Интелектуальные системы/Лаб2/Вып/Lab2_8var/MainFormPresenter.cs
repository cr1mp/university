...
namespace Lab2_8var
{
  class MainFormPresenter
  {
    ...
    /// </summary>
    /// <param name="workFacts"></param>
    /// <param name="workTruths"></param>
    private void GetReverseWorkIterations(ArrayList workFacts, ArrayList workTruths)
    {
      ...
      while (flag1)
      {
        ...
        {
          rule1 = ResolveConflict(node1, arrayList4, arrayList5, rule1, ref str3);
        }

        if (rule1 != null)
        {
          var node3 = new TreeNode("Сработало правило: " + rule1.Name + " : " + rule1.Truth + " [" + str3 + "]");
          node1.Nodes.Add(node3);
          var node4 = new TreeNode(rule1.Condition);
          node3.Nodes.Add(node4);
          ...
          foreach (int fact_id in rule1.Conclusions)
          {
            ...
            if (factById != null)
            {
              var fact = new Fact(-1, factById.Object, factById.Attribute, factById.Value, rule1.Truth, factById.Type);
    ...
    /// <param name="arrayList4"></param>
    /// <param name="arrayList5"></param>
    /// <param name="rule1"></param>
    /// <param name="str3"></param>
    /// <returns></returns>
    private Rule ResolveConflict(TreeNode node1, ArrayList arrayList4, ArrayList arrayList5, Rule rule1, ref string str3)
    {
      var node3 = new TreeNode("Конфликтное множество");
      node1.Nodes.Add(node3);
      for (int index1 = 0; index1 < arrayList4.Count; ++index1)
      {
        Rule rule2 = (Rule)arrayList4[index1];
        var node4 = new TreeNode(rule2.Name + " : " + rule2.Truth + " [" + (string)arrayList5[index1] + "]");
        node3.Nodes.Add(node4);
        var node5 = new TreeNode(rule2.Condition);
        ...
        for (int index2 = 0; index2 < rule2.Conclusions.Count; ++index2)
        {
          ...
          if (factById != null)
          {
            var node6 = new TreeNode(factById.ToString());
